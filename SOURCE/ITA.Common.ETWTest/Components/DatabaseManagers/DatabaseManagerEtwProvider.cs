...
{    
    ...
    /// Static ETW provider
    /// </summary>
    [EventSource(Name = "ITA-ETWTest-DatabaseManager", LocalizationResources = "ITA.Common.ETWTest.Localization.Messages")]
    public class DatabaseManagerEtwProvider : EventSource, IStaticEventSource
    {
        private static readonly Lazy<DatabaseManagerEtwProvider> Instance = new Lazy<DatabaseManagerEtwProvider>(() => new DatabaseManagerEtwProvider());

        public static DatabaseManagerEtwProvider Log
        {
        ...
        public class Tasks
        {
            ...
            public const EventTask Database = (EventTask)0x0008;
        }

        public class Opcodes
        {
            public const EventOpcode LoadCpu = (EventOpcode) EventId.LoadCpu;
        }
        ...
        [Event(EventId.LoadCpu, Keywords = Keywords.Trace, Level = EventLevel.Verbose, Task = Tasks.Database, Opcode = Opcodes.LoadCpu)]
        public void LoadCpu(int seconds)
        {
            if (IsEnabled()) WriteEvent(EventId.LoadCpu, seconds);
        }
    } 
}
